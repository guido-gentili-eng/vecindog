...
function ModalPuntuacion({ cuidadorPostId, inicial, onGuardado, onClose }: {
  cuidadorPostId: string; inicial: CuidadorRating | null; onGuardado: () => void; onClose: () => void;
}) {
  const { t } = useLanguage();
  ...
  async function handleSubmit() {
    ...
    try {
      await calificarCuidador({
        cuidadorPostId, estrellas, cuidado_animal: cuidadoAnimal,
    ...
    <Modal visible transparent animationType="fade" onRequestClose={onClose}>
      <View style={styles.modalOverlay}>
        <ScrollView contentContainerStyle={styles.modalSheet}>
          <Text style={styles.modalTitle}>{t.cuidadorModalTitle}</Text>
...
export default function PerfilCuidadorScreen() {
  ...
  async function cargar() {
    setCargando(true);
    const [p, { ratings: rs, resumen: res }] = await Promise.all([obtenerPost(id), getRatingsCuidador(id)]);
    setPost(p);
    setRatings(rs);
    setResumen(res);
    setCargando(false);
  ...
  if (!post) {
    return (
      <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', gap: 8 }}>
        ...
        <TouchableOpacity onPress={() => router.replace('/cuidado' as any)}><Text style={{ color: '#0d9488', fontWeight: '700' }}>{t.cuidadoVolver}</Text></TouchableOpacity>
      </View>
    );
  }
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ padding: 20, paddingTop: 56, paddingBottom: 60 }}>
      ...
      <View style={styles.headerCard}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={styles.nombre}>{post.nombre ?? t.cuidadorFallbackNombre}</Text>
      ...
      {modalAbierto && (
        <ModalPuntuacion cuidadorPostId={id} inicial={resumen?.miRating ?? null} onGuardado={handleGuardado} onClose={() => setModalAbierto(false)} />
      )}
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  back:       { fontSize: 14, fontWeight: '700', color: '#0d9488', marginBottom: 12 },
  headerCard: { flexDirection: 'row', gap: 14, backgroundColor: Colors.white, borderRadius: 20, padding: 18 },
  avatar:     { width: 64, height: 64, borderRadius: 18, backgroundColor: '#ccfbf1', alignItems: 'center', justifyContent: 'center' },
  ...
  waBtn:      { backgroundColor: '#0d9488', borderRadius: 16, paddingVertical: 13, alignItems: 'center', marginTop: 12 },
  ...
  calificarBtn: { backgroundColor: '#0d9488', borderRadius: 14, paddingHorizontal: 14, paddingVertical: 8 },
  calificarBtnText: { color: Colors.white, fontWeight: '800', fontSize: 12 },
  guardadoText: { fontSize: 13, fontWeight: '700', color: '#0d9488', marginBottom: 10 },
  emptyBox:   { borderWidth: 1, borderStyle: 'dashed', borderColor: '#5eead4', backgroundColor: '#f0fdfa', borderRadius: 16, padding: 24, alignItems: 'center' },
  emptyText:  { fontSize: 13, color: '#0d9488' },
  ratingCard: { backgroundColor: Colors.white, borderRadius: 16, padding: 14 },
  ratingDate: { fontSize: 11, color: Colors.inkMuted },
  badge:      { backgroundColor: '#ccfbf1', borderRadius: 20, paddingHorizontal: 10, paddingVertical: 4 },
  ...
  saveBtn:    { flex: 1, backgroundColor: '#0d9488', borderRadius: 16, paddingVertical: 12, alignItems: 'center' },
