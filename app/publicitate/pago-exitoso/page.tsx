...
export default function PagoExitosoPage() {
  ...
  useEffect(() => {
    ...
    // Activar los ads automáticamente
  ...
  if (cargando) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4">
        ...
        <p className="font-bold text-ink">Activando tu anuncio…</p>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-md py-16 text-center">
      <div className="card p-8 md:p-10">
        ...
        <h1 className="mt-5 font-display text-2xl font-black text-ink md:text-3xl">
          {pending ? '¡Pago en proceso!' : '¡Tu anuncio está activo!'}
        </h1>

        <p className="mt-2 text-ink-muted">
          {pending
            ? 'Tu pago está siendo procesado. Cuando se confirme tu anuncio se activa automáticamente.'
        ...
        <div className="mt-5 rounded-2xl bg-brand-cream p-4 text-left text-sm text-ink-muted">
          ...
          <p className="mt-1">¿Necesitás hacer un cambio en tu anuncio? Escribinos a <strong>hola@mivecindog.com.ar</strong> o por WhatsApp al <strong>+54 9 291 405-0210</strong></p>
        </div>

        <Link href="/" className="mt-6 inline-flex items-center gap-1 text-sm font-bold text-brand-primary hover:underline">
          Ver mi anuncio en la app <ArrowRight className="h-3.5 w-3.5" />
        </Link>
      </div>
    </div>
  );
