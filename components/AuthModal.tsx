...
import { Dog, Mail, Lock, AlertCircle, Loader2, Eye, EyeOff, CheckCircle2 } from 'lucide-react';
import { useAuth } from '@/contexts/AuthContext';

export default function AuthModal() {
  const { hasChosen, loading, signIn, signUp, enterAsGuest } = useAuth();
  ...
  /* No mostrar si ya eligió o todavía cargando */
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (mode === 'login') {
        ...
        // si no needsConfirm, el onAuthStateChange ya actualiza el user
      }
    } finally {
      setSubmitting(false);
    }
...
function tradError(msg: string): string {
  ...
  return msg; // fallback: mostrar mensaje original
}
