...
export async function getRatingsCuidador(cuidadorPostId: string): Promise<{ ratings: CuidadorRating[]; resumen: ResumenRating }> {
...
export async function calificarCuidador(input: {
  ...
  const { data: post } = await supabase.from('posts').select('user_id').eq('id', input.cuidadorPostId).single();
