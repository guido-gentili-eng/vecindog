...
export async function getRatingsTransportador(transportadorPostId: string): Promise<{ ratings: TransportadorRating[]; resumen: ResumenRating }> {
...
export async function calificarTransportador(input: {
  ...
  const { data: post } = await supabase.from('posts').select('user_id').eq('id', input.transportadorPostId).single();
  if (post?.user_id === user.id) throw new Error('No podés calificar tu propia publicación.');
