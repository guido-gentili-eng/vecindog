...
// SVG con estilos inlineados (sin CSS classes para que sharp lo procese bien)
const svgSource = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 59.57 89.72">
...
// El pin es vertical (59x89), lo centramos en un canvas cuadrado con fondo blanco
async function makeIcon(size, outPath) {
  // padding: el pin ocupa ~75% del alto, centrado
  const pinH = Math.round(size * 0.82);
  const pinW = Math.round(pinH * (59.57 / 89.72));
  const offsetX = Math.round((size - pinW) / 2);
  const offsetY = Math.round((size - pinH) / 2);

  const svgBuf = Buffer.from(svgSource);

  const pinPng = await sharp(svgBuf)
    .resize(pinW, pinH)
    .png()
    .toBuffer();
  ...
  await sharp({
    ...
    .composite([{ input: pinPng, left: offsetX, top: offsetY }])
    .png()
    .toFile(outPath);
...
for (const s of sizes) {
  await makeIcon(s, `public/icons/icon-${s}x${s}.png`);
}
// apple-touch-icon y favicons
await makeIcon(180, 'public/apple-touch-icon.png');
await makeIcon(32,  'public/favicon-32x32.png');
await makeIcon(16,  'public/favicon-16x16.png');
